...
for (i in 1:1000){
  muestra_dep[i,1:100] <- lgc(m=100-1,a=2*i,c=10*1,z0=i)
}
...
### **5.2** Alteración de supuestos: No identicamente distribuidas
...
for (i in 1:1000){
  ...
  for (j in 1:100){
    muestras_id$datos[[i]][j] <- rt(1, j, 1/j)
  }
...
mean = mu
sd = sigma 
binwidth = .05 # passed to geom_histogram and stat_function 
...
ggplot(df, aes(x = as.numeric(muestras_id$medias), mean = mean, sd = sd, binwidth = binwidth, n = n)) +
  ...
  scale_x_continuous(expand = c(0, 0)) + ggtitle("Histograma de las medias") +
  scale_color_discrete(name = "", 
                       labels = c("a" = expression(t~de~Student),
                                  "b" = expression(Normal))) + 
  xlab("Media") + ylab("Frecuencia") + 
  theme_bw(base_size = 16) + theme(legend.position = c(0.8, 0.8), legend.text.align = 0)
```
